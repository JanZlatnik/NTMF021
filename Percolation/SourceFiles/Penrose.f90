...
MODULE Penrose
    IMPLICIT NONE
    PRIVATE 
    PUBLIC :: penrose_substitution, construct_grid, phi, iphi, ophi, generate_boundary
    ...
    SUBROUTINE generate_boundary(nodes, n_iter, xtip, ytip, boundary_flags)
        IMPLICIT NONE
        REAL(8), DIMENSION(:,:), INTENT(IN)                 :: nodes
        INTEGER, INTENT(IN)                                 :: n_iter
        REAL(8), INTENT(IN)                                 :: xtip, ytip
        LOGICAL, DIMENSION(:,:), ALLOCATABLE, INTENT(OUT)   :: boundary_flags

        INTEGER :: i, n_nodes
        REAL(8) :: slope, intercept, astand, amin, eps, x, y


        astand = SQRT(xtip**2 + ytip**2) / phi**n_iter
        amin = astand * iphi
        eps = amin / 100.0d0


        n_nodes = SIZE(nodes, 1)
        IF (ALLOCATED(boundary_flags)) DEALLOCATE(boundary_flags)
        ALLOCATE(boundary_flags(n_nodes, 2))
        ...
        DO i = 1, n_nodes
            x = nodes(i,1)
            y = nodes(i,2)
            IF (ABS(y + (slope * x + intercept)) < eps .AND. x <= 0.0d0 .AND. y <= 0.0d0) THEN
                boundary_flags(i,1) = .TRUE.
            END IF
            IF (ABS(y + (slope * x) - intercept) < eps .AND. x >= 0.0d0 .AND. y >= 0.0d0) THEN
                boundary_flags(i,2) = .TRUE.
            END IF
        END DO
